...
class AuthorApi {
  ...
  Future<Either<HttpRequestFailure, List<Author>>> getAuthors({
    ...
    try {
      final response = await get(
          Uri.parse('$_baseUrl/authors/$pageNumber/$numberOfPostsPerRequest'));

      if (response.statusCode == 200) {
        final responseList = jsonDecode(response.body) as Map<String, dynamic>;

        final List<AuthorIiap> authorsIiap =
            getAuthorIiapList(responseList['autores']);
  ...
  Future<void> getIndigenousCommunitiesSearchGeneral(
    ...
    try {
      ...
      final responseList = jsonDecode(response.body) as Map<String, dynamic>;
      final List<AuthorIiap> authorsIiap =
          getAuthorIiapList(responseList['authors']);
      final List<Author> authors = authorsIiap
          .map(
            (author) => _authorMapper.authorIiapToAuthor(author),
          )
          .toList();
  ...
  Future<Either<HttpRequestFailure, Author>> getIndigenousCommunityId(
      String id) async {
    try {
      final response =
          await get(Uri.parse('$_baseUrl/authors/$id'));
